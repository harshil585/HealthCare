...
const Notifications = () => {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      setNotifications([
        { notificationId: 1, message: "Welcome Dr. Practitioner! You have pending slots scheduled.", isRead: false, createdAt: new Date().toISOString() }
      ]);
    } finally {
      setLoading(false);
    }
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 40, borderBottom: `1px solid ${border}`, paddingBottom: 20 }}>
                {['ALL', 'UNREAD', 'READ'].map(tab => (
                  <button
                    ...
                    style={{
                      padding: "10px 20px",
                      background: filter === tab ? "rgba(16,185,129,0.1)" : "transparent",
                  ...
                  <AnimatePresence>
                    {filtered.map((item) => (
                      <motion.div
                        ...
                        exit={{ opacity: 0, x: 20 }}
                        style={{
                          background: item.isRead ? 'transparent' : 'rgba(16,185,129,0.02)',
                          border: `1px solid ${item.isRead ? border : 'rgba(16,185,129,0.15)'}`,
                          borderRadius: 24,
                          padding: '24px 30px',
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      >
                        <div style={{ 
                          ...
                          height: 48, 
                          borderRadius: 16, 
                          background: item.isRead ? 'rgba(148,163,184,0.08)' : 'rgba(16,185,129,0.08)', 
